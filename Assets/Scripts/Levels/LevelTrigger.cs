...
public class LevelTrigger : MonoBehaviour
{
    ...
    private void Awake()
    {
        _levels = new[] { GameObject.Find("Lobby"), GameObject.Find("Level1"), GameObject.Find("Level2"), 
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Player") && (levelIndex == -1 || GameManager.Instance.IsLevelUnlocked(levelIndex)))
        {
            ...
            switch (levelIndex)
            {
                ...
                for (int i = 1; i < _levels.Length; i++) _levels[i]?.SetActive(false);
                //SetActiveCamera("VCam_Lobby");
                break;
                case 0: // Nivel 1
                    other.transform.position = new Vector3(29.156f, -6.098f, 0f);
                    _levels[1]?.SetActive(true);
                    _levels[0]?.SetActive(false);
                    for (int i = 2; i < _levels.Length; i++) _levels[i]?.SetActive(false);
                    //SetActiveCamera("VCam_Level1");
                    break;
                case 1: // Nivel 2
                    other.transform.position = new Vector3(27.7f, 16.4f, 0f);
                    _levels[2]?.SetActive(true);
                    for (int i = 0; i < _levels.Length; i++) if (i != 2) _levels[i]?.SetActive(false);
                    //SetActiveCamera("VCam_Level2");
                    break;
                case 2: // Nivel 3
                    other.transform.position = new Vector3(60f, 0f, 0f);
                    _levels[3]?.SetActive(true);
                    for (int i = 0; i < _levels.Length; i++) if (i != 3) _levels[i]?.SetActive(false);
                    //SetActiveCamera("VCam_Level3");
                    break;
                case 3: // Nivel 4
                    other.transform.position = new Vector3(80f, 0f, 0f);
                    _levels[4]?.SetActive(true);
                    for (int i = 0; i < _levels.Length; i++) if (i != 4) _levels[i]?.SetActive(false);
                    //SetActiveCamera("VCam_Level4");
                    break;
                case 4: // Nivel 5
                    other.transform.position = new Vector3(100f, 0f, 0f);
                    _levels[5]?.SetActive(true);
                    for (int i = 0; i < 5; i++) _levels[i]?.SetActive(false);
                    //SetActiveCamera("VCam_Level5");
                    break;
            }
            
            // Actualiza la cámara tras teletransportar
            CameraFollow cameraFollow = FindObjectOfType<CameraFollow>();
            if (cameraFollow != null)
            {
                cameraFollow.player = other.transform; // Asegura que siga al jugador
                cameraFollow.UpdateCameraPosition(); // Ajusta la posición
            }
            else
            {
                Debug.LogError("No se encontró el componente CameraFollow en la escena!");
            }

            // Asegura que la cámara siga al jugador
            /*CinemachineVirtualCamera activeCam = Array.Find(_vcams, v => v.name == GetCameraName(levelIndex));
